        ...
        for t in range(self.seq_length + group_size - 1):
            for divm in range(group_size): 
                if t >= divm and t <= self.seq_length + divm - 1:
                    ...
                    logprobs_table[divm], state_table[divm] = self.get_logprobs_state(it.to(utils.device), *(args[divm] + [state_table[divm]]))
        ...
        for t in range(self.seq_length + group_size - 1):
            for divm in range(group_size): 
                if t >= divm and t <= self.seq_length + divm - 1:
                    ...
                    # suppress previous word
                    if decoding_constraint and t-divm > 0:
                        logprobsf.scatter_(1, beam_seq_table[divm][t-divm-1].unsqueeze(1).to(utils.device), float('-inf'))
                    if remove_bad_endings and t-divm > 0:
    ...
    def sample_next_word(self, logprobs, sample_method, temperature):
        if sample_method == 'greedy':
            ...
            def sample_gumbel(shape, eps=1e-20):
                U = torch.rand(shape).to(utils.device)
